...
# JavaFX JNI Test Runner Script (FIXED)
# Runs ExplicitJNITest with proper environment and classpath

set -e

echo "=========================================="
echo "JavaFX JNI Test Runner (FIXED)"
...
if [ -n "$JAVAFX_BUILD_DIR" ]; then
    ...
    echo "Please run install_fixed.sh first to build JavaFX"
    exit 1
fi
...
if [ ! -d "$JAVA_HOME" ]; then
    ...
    echo "Please install OpenJDK 17 or run install_fixed.sh"
    exit 1
fi
...
if ! check_file "$JAVAFX_BASE_JAR" "JavaFX Base JAR"; then
    ...
    JAVAFX_BASE_JAR=$(find "$JAVAFX_BUILD_DIR" -name "javafx.base.jar" | head -1)
    
    if [ -z "$JAVAFX_BASE_JAR" ]; then
        echo "❌ Could not find javafx.base.jar"
        REQUIRED_FILES_FOUND=false
    else
        echo "✅ Found base JAR at: $JAVAFX_BASE_JAR"
    fi
fi
...
if ! check_file "$EXPLICIT_TEST" "ExplicitJNITest.java"; then
    ...
    elif [ -f "$JAVAFX_BUILD_DIR/ExplicitJNITest.java" ]; then
        EXPLICIT_TEST="$JAVAFX_BUILD_DIR/ExplicitJNITest.java"
        echo "✅ Found ExplicitJNITest.java in build directory"
    else
...
if [ "$REQUIRED_FILES_FOUND" = false ]; then
    ...
    echo "  1. Run install_fixed.sh to build JavaFX"
...
if [ $SO_COUNT -gt 0 ]; then
    ...
    echo "⚠️  No native libraries found in $JAVAFX_LIB_DIR"
    ...
    if [ -d "$SYSTEM_JNI_DIR" ]; then
        ...
        if [ $SYSTEM_SO_COUNT -gt 0 ]; then
            ...
            echo "Native JNI tests will likely fail"
        fi
    else
...
# Copy ExplicitJNITest.java to current directory if needed
if [ "$EXPLICIT_TEST" != "./ExplicitJNITest.java" ]; then
    cp "$EXPLICIT_TEST" ./ExplicitJNITest.java
    echo "✅ Copied ExplicitJNITest.java to current directory"
    EXPLICIT_TEST="./ExplicitJNITest.java"
fi
...
java -cp "$CLASSPATH" \
     ...
     -Djavafx.lib.path="$JAVAFX_LIB_DIR" \
     -Dprism.order=sw \
...
if [ $TEST_RESULT -eq 0 ]; then
    ...
    echo "  • Check that install_fixed.sh completed successfully"
